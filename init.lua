...
require('lazy').setup({
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    dependencies = {
      ...
      { 'williamboman/mason.nvim', config = true }, -- Automatically calls require('mason').setup()
...

      {
        ...
        dependencies = {
          'nvim-lua/plenary.nvim', -- Already a dependency of Telescope, but good to be explicit
          'stevearc/dressing.nvim', -- Add this dependency
        },
        -- We call setup manually within lspconfig's config
      },
    },
    config = function()
      -- Keep your LSP explanation comments if you like them

      -- Keep the LspAttach autocommand setup (this handles on_attach for ALL LSPs)
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- Keep the document highlight and inlay hint logic
      ...
      -- Keep your existing diagnostic config
      vim.diagnostic.config {
        severity_sort = true,
      ...
      -- Keep your capabilities setup
      local capabilities = vim.lsp.protocol.make_client_capabilities()
      capabilities = vim.tbl_deep_extend('force', capabilities, require('cmp_nvim_lsp').default_capabilities())

      -- Define servers for the *generic* handler (DO NOT add dartls or tailwindcss here)
      local servers = {
      ...
      vim.list_extend(ensure_installed, {
        'stylua', -- Keep your formatters
        -- 'dartls',         -- ADD dartls
        'tailwindcss', -- ADD tailwindcss
        -- Add 'flutter-debug-adapter' or other tools if needed
      })
      require('mason-tool-installer').setup { ensure_installed = ensure_installed } -- Keep this call

      -- Keep the generic mason-lspconfig setup
      require('mason-lspconfig').setup {
        ensure_installed = '',
        automatic_installation = true,
        handlers = {
          -- Generic handler for servers in the `servers` table
          function(server_name)
            local server = servers[server_name] or {}
            server.capabilities = vim.tbl_deep_extend('force', {}, capabilities, server.capabilities or {})
            require('lspconfig')[server_name].setup(server)
          end,
          -- Keep specific handlers if you have them (like the example ruff_lsp one from kickstart)
          -- Make sure this doesn't conflict with the `servers` table entry if you have one
          -- ruff_lsp = function()
          --   require('lspconfig').ruff_lsp.setup {
          --     capabilities = capabilities,
          --     -- add other specific ruff settings here if needed
          --     -- init_options = { settings = { args = {} } },
          --   }
          -- end,
          -- Ensure NO handlers for dartls or tailwindcss exist here
        },
      }
